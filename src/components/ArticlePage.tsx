    ...
    <>
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <nav className="mb-6 text-sm text-mantra-muted flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/${post.urlPrefix}/`} className="hover:text-mantra-red transition-colors capitalize">
            {post.urlPrefix}
          </Link>
          <span>/</span>
          <span className="text-mantra-dim truncate max-w-xs">{post.title}</span>
        </nav>

        {/* Header */}
        <header className="mb-8">
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <span className={`px-2.5 py-0.5 text-xs font-medium uppercase tracking-wider rounded border ${TYPE_BADGE[post.contentType] || TYPE_BADGE.news}`}>
              {post.contentType}
            </span>
